...
# Copyright (c)2004,2012,2013 by Brian Manning.
...
if [ ${OSDETECT} == "Darwin" ]; then
    ...
    TEMP=$(/usr/bin/getopt -o hqv:o: \
        --long help,quiet,verbose:,option: -n '${SCRIPTNAME}' -- "$@")
else
...
echo "OPTION is currently set to $OPTION"
